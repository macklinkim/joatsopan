...
1. **보안/법적(High)**: /api 레이트리밋(@upstash 또는 미들웨어)·OG 임의생성 남용 방어, robots/X-Robots, 시상식 라벨 완화, 정정창구.
2. **제품 P1**: 추천 2분화+배수(최저비용·고효과), 공유 복사/카카오 버튼, 위험도 사다리, "그때 vs 지금"(시계열 합성 고지 선행), 상세 기준월 노출.
